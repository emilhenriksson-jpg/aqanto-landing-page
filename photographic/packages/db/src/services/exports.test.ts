...
class StreamingTestStore implements BlobStore {
  ...
  async createUpload(): Promise<BlobUpload> {
    ...
    // Arrow functions rather than `const self = this`: they take `this` lexically from
    // `createUpload`, so the counters below are the instance's own without an alias.
    return {
      write: async (chunk) => {
        hash.update(chunk);
        byteSize += chunk.byteLength;
        this.uploaded += chunk.byteLength;
      },
      complete: async () => {
        this.completed += 1;
        this.uploadChecksum = hash.digest('hex');
        return {
          key: 'exports/streamed.zip',
          checksum: this.uploadChecksum,
          byteSize,
          deduplicated: false,
        };
      },
...
describe('the archive is never resident', () => {
  ...
  it('abandons the half-written object when a build fails', async () => {
    ...
    const result = await new PgExports(pool, broken, {
