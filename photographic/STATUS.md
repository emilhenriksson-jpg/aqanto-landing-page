...
- 2026-09-24 — Root: investigated the actual ChatGPT phone Voice conversation. Verified a typed `get_context` call, not a fresh Voice write. Added correlated MCP catalogue/request/response diagnostics without new content or credential fields; changed OAuth client/scopes/room scope now require a fresh session instead of retaining initialized permissions. Added transport continuity, reviewed save/readback, scope denial, refresh and privacy regressions. All 80 MCP tests, MCP typecheck and changed-file lint pass. Corrected categorical Voice claims; documented the ten handoff questions and the still-pending physical phone acceptance test in `docs/chatgpt-voice-verification.md`. This is not a verified phone Voice fix.

- 2026-09-24 — Root: inspected the actual native-origin conversation; app selection alone still produced a greeting and availability claim, with onboarding only after the user's explicit request. Desktop launch now includes a short visible read-and-propose request alongside the account's app mention. Moved the startup sequence into the first 512 instruction characters and distinguished pending proposals from paused/already-presented offers. Local agent/connect/MCP tests (306), ChatStart tests (23), full typecheck and changed-file lint pass. Native model initiative remains an acceptance check, not established by these tests. See `docs/chatgpt-start-verification.md`.
...
- Local validation: core 142, agent 82, connect 147, MCP 67 and web 139 tests passed; package typechecks passed. Chrome desktop/narrow-layout review passed, including help, navigation, local receipt dates and no console errors. External native model replies remain a manual acceptance check. Follow-up on 2026-09-20: updated the end-to-end assertions for the new recent-memory heading and live compass defaults; all 89 memory-backed acceptance tests and the e2e typecheck/lint pass.
