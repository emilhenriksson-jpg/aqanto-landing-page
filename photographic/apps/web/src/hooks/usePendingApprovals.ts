...
export function usePendingApprovals(): ApprovalItem[] | null {
  ...
  useEffect(() => {
    ...
    // `refresh` swallows its own failures, so the handler is belt and braces rather than
    // the only thing standing between a failed read and an unhandled rejection. It stays
    // because `void` does not: the linter is configured with `ignoreVoid: false`, after a
    // `void promise` with no handler took the server process down once.
    if (!asked) refresh().catch(() => {});
    return () => {
