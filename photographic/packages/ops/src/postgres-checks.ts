...
export const MIGRATION_ARTIFACTS: Record<string, string> = {
  ...
  '0005_swedish_search.sql': index('item_fts_idx'),
  ...
  '0017_lifecycle_atomicity.sql': index('account_deletion_claimable_idx'),
  '0018_job_leases_and_export_durability.sql': fn('reclaim_expired_jobs'),
  '0019_document_lifecycle_and_upload_state.sql': `to_regclass('app.blob_upload') IS NOT NULL`,
  // A role rather than a relation, and the only entry here that is not a schema object.
  // `pg_roles` is readable by every role, so this costs nothing and works when the check
  // runs as the application itself. It does not assert the grants -- that is
  ...
  // The column rather than the index, and for the reason this file exists: the index is a
  // performance aid the backfill would survive without, while `embedding_model` is what
  // says which model produced a vector. A restore that stamped this migration without
...
/**
 * Migrations that create nothing, and why that is not an oversight.
 *
 * Separate from `MIGRATION_ARTIFACTS` rather than given a placeholder there, because the
 * two statements are different: "here is the object this migration made" and "this
 * migration made no object". A placeholder would let a real omission hide as a deliberate
 * one, which is the failure this whole file exists to prevent.
 */
export const MIGRATIONS_WITHOUT_ARTIFACT: Record<string, string> = {
  '0016_app_role_grants.sql':
    'Grants only, and guarded on the role pre-existing, so on a database without ' +
    '`photographic_app` it is a no-op that creates nothing to check. Superseded by the ' +
    'unguarded grants migration, which does have an artifact: the role itself.',
};
